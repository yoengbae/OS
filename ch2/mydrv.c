...
static ssize_t mydrv_write(struct file* file, const char* buf, size_t count, loff_t* ppos){
	...
	for(i=0; i<count; i++){
		if(mydrv_data[mydrv_write_offset+i]>='a' && mydrv_data[mydrv_write_offset+i]<='z')
			mydrv_data[mydrv_write_offset+i]-=32;
		else if(mydrv_data[mydrv_write_offset+i]>='A' && mydrv_data[mydrv_write_offset+i]<='Z')
			mydrv_data[mydrv_write_offset+i]+=32;
	}
	
	mydrv_write_offset += count;
	if(mydrv_write_offset>=100)
